...
STATUS: OK
Last run: 2026-08-26 (cron, iter 23 — MBR migration slice 3: CIP cleaning economics + cleaning-due advisory; gates: build ✅ tsc ✅ sim ✅ ui 70+60 ✅ eng 342/342 ✅)
...
Next goal (iter 24): MBR migration SLICE 4 — cleaning *outage* + membrane end-of-life economics: (a) a CIP event takes the train offline for a documented window (e.g. `offlineHours` on MbrFoulingState, engine bypasses permeate production while >0) so cleaning has a throughput cost, not just a fee; (b) replacement CAPEX when cumulative irreversible fouling/lifetimeYears is exhausted (§R lifetimeYears is already in the catalog). Then post-mission freedom pool (blower VFD tree, sludge-line objectives, placement SFX) or V2-B story/environment work.
